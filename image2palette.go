...
import (
	...
	"bramp.net/dataflow-art/morebeam"
...
var (
	...
	index         = flag.String("index", "all_data_info_sample.csv", "Index of the art.")
	outputPrefix  = flag.String("output", "output", "Output file (required).")
	paletteColors = flag.Int("palette", 6, "Number of colors in the color palette.")
)

func init() {
	beam.RegisterType(reflect.TypeOf((*calculateColorPaletteFn)(nil)).Elem())
...
// ExtractHistogram calculates the paintings color histogram and adds it to the Painting Metadata.
...
func CalculateColorPalette(s beam.Scope, hists beam.PCollection) (beam.PCollection, beam.PCollection) {
	...
	return beam.ParDo2(s, &calculateColorPaletteFn{
		PaletteColors: *paletteColors,
	}, hists)
}

type calculateColorPaletteFn struct {
	PaletteColors int `json:"colors"` // Number of colors requested in the palette.
}

func (fn *calculateColorPaletteFn) ProcessElement(ctx context.Context, result HistogramResult, emit func(HistogramResult), errors func(string, string)) {
	...
	pal, err := extractor.FromHistogram(result.Histogram, fn.PaletteColors)
	if err != nil {
...
func WriteIndex(s beam.Scope, indexFilename string, results beam.PCollection) {
	...
	objs := beam.ParDo(s.Scope("MarshalToJson"), func(result HistogramResult) (string, error) {
		key, err := json.Marshal(result.Key)
		if err != nil {
			return "", err
		}
...
func GeneratePalette(s beam.Scope, group string, keyFn func(Painting) string, paintings beam.PCollection) beam.PCollection {
	...
	// PCollection<Painting> -> PCollection<string, Painting>
	...
	// PCollection<CoGBK<string, Painting>> -> PCollection<string, ColorHistogram>
	histograms, errors1 := ExtractHistogram(s, paintingsByGroup)

	// Calculate the color palette for the combined histograms.
	// PCollection<string, ColorHistogram> -> PCollection<string, []color.RGBA>
	...
	// PCollection<string, []color.RGBA> -> nothing
...
func main() {
	...
	errors3 := GeneratePalette(s, "year", func(art Painting) string {
		return art.Decade()
	}, paintings)
